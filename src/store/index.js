...
const store = new Vuex.Store({
   ...
   mutations: {
      ...
      updateMusicId(state, musicId) {
         ...
         localStorage.setItem('lastMusicId', musicId);
      },
      // 修改播放状态
      ...
      updateMusicList(state, payload) {
         ...
         if (typeof payload.musicListId !== 'string' && payload.musicListId) {
            localStorage.setItem('lastMusicListId', payload.musicListId);
         }
      ...
      insertRecords(state, record) {
         let index;
         if ((index = state.records.findIndex(item => item.id === record.id)) !== -1) {
